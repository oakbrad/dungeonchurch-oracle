...
function fitTextInCircle(text, title, radius) {
    ...
    if (tryTruncateText(text, title, radius, padding)) {
        return true;
    }
...
function tryTruncateText(text, title, radius, padding) {
    // Try with minimum font size
    const fontSize = Math.max(minFontSize, Math.min(10, radius * 0.6));
    text.style("font-size", fontSize + "px");
